...
unset pv
while [ -z ${pv} ]; do
read -p "Please Enter IP: " pv
done
echo ""
external_ip_line="externalip=$pv"
...
cd ~
mkdir ./.3dcoin$COUNT
echo "$config" > ./.3dcoin$COUNT/3dcoin.conf
cd ~
cd /usr/local/bin
mkdir Masternode
cd Masternode
...
@reboot service sshd start
@reboot service fail2ban start"
...
#		echo ""
#		echo " # 3Dcoin Masternode MULTI installation Original with PRECOMPILED DAEMON #"
#		Config_Masternode
#		check_swap
#		prep_3dcoin_core	
#		install_3dcoin_core_COMP
...
while true; do
		...
		sleep 5
		cd ~
		cp -r .3dcoin/ .3dcoin$COUNT/ 	
		echo "$config" > ./.3dcoin$COUNT/3dcoin.conf
		cd ~
		crontab -l > /tmp/cron2fix 
		echo "@reboot 3dcoind -datadir=/root/.3dcoin$COUNT -daemon" >> /tmp/cron2fix 
